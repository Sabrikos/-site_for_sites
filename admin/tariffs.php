...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!appCsrfValid('admin_csrf', (string) ($_POST['csrf_token'] ?? ''))) {
        ...
        $price = filter_var($_POST['price'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]]);
        $active = isset($_POST['active']) ? 1 : 0;

        if ($id === false || $price === false || $name === '' || mb_strlen($name, 'UTF-8') > 100) {
            $error = 'Проверьте название и цену тарифа.';
        } else {
            $statement = $pdo->prepare('UPDATE tariffs SET name = ?, description = ?, price = ?, active = ? WHERE id = ?');
            $statement->execute([$name, $description !== '' ? $description : null, (int) $price, $active, (int) $id]);
...
$rows = $pdo->query(
    'SELECT t.id, t.name, t.description, t.price, t.active, s.name AS service_name
     FROM tariffs t
